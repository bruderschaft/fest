var compile = (function(){
	...
	function compileExpr(expr, _getExpr){
		var i = 0, exprValue = '', result = {containsExpr: false, text: []}, isObject = (typeof(expr) === 'object');
		exprValue = (isObject) ? expr.value : expr;
		exprValue = exprValue.replace(/{{/g, "__DOUBLE_LEFT_CURLY_BRACES__").replace(/}}/g, "__DOUBLE_RIGHT_CURLY_BRACES__");
		exprValue.match(/{[^}]*}|[^{}]*/g).forEach(function (str) {
			if (str !== '') {
				if (str[0] === '{') {
					i++;
					result.text.push('(' + str.slice(1, -1).replace(/__DOUBLE_LEFT_CURLY_BRACES__/g, "{").replace(/__DOUBLE_RIGHT_CURLY_BRACES__/g, "}") + ')');
					if (isObject){
						_getExpr(result.text[result.text.length-1], 'expression #' + (i) + ' in attribute "' + expr.name + '"');
					}
	...
	function _compile (data){
		...
		parser.ontext = parser.oncdata = function (text) {
			...
			if (stack[stack.length - 1] === 'script') {
				...
				var expression = compileExpr(text, _getExpr);
				if (expression.containsExpr && options.expand_text) {
					section.source += '__fest_pushstr(__fest_context,' + expression.text + ');';
				} else{
